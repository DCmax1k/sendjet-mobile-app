...
class DashHome extends Component {
  ...
  async addConversation(users) {
    ...
    const response = await sendData('/messages/createconversation', convoData);
  ...
  async unaddFriend(friend) {
    ...
    const response = await sendData('/search/unadduser', { id: friend._id });
  ...
  async declineFriendRequest(friend) {
    ...
    if (this.state.user.friendRequests.length === 1) this.focusWidget('');
    const response = await sendData('/search/declinefriendrequest', {id: friend._id});
